...
description: >
  ...
  Also supports explicit updates with /team-knowledge update.
---
...
**Flags:**
- `--force` — Run even if no changes detected since last update
- `--dry-run` — Show the change summary without calling the update agent
...
3. Copy the template from the templates directory (see Installation)
...
1. Read the state file (`.team-knowledge-state` next to TEAM-KNOWLEDGE.md)
2. Report:
   - Last commit processed (hash + short message)
   - Last PR number processed
   - Last update date
   - Current repo HEAD vs last processed (are we behind?)

## Script Location

The update script is searched in this order:
1. Path specified in `.team-knowledge.conf` (`SCRIPT_PATH` variable)
2. Same directory as the TEAM-KNOWLEDGE.md file (`update-team-knowledge.sh`)
3. `~/.claude/scripts/update-team-knowledge.sh` (global install)
...
```bash
# .team-knowledge.conf — Team Knowledge configuration
KNOWLEDGE_FILE="../TEAM-KNOWLEDGE.md"     # Path to knowledge file (relative to repo root)
STATE_FILE="../.team-knowledge-state"     # Path to state file
BASE_BRANCH="dev"                         # Main branch (default: auto-detect)
PROJECT_NAME="my-project"                 # Project name for logs
SCRIPT_PATH="../update-team-knowledge.sh" # Path to update script
```

All paths can be relative (resolved from repo root) or absolute.
...
- If the script can't find the repo, run from inside the git repo or set `REPO_DIR`
